...
Tamed creatures newly summoned directly by a Blood Magic staff can also create friendly sub-summons through `SpawnAbility`. Their immediate child creatures are tamed and use the `PlayerSpawned` faction, with a fixed maximum of two living children per parent and no additional YAML or config options. Further creation is skipped until a child dies or is confirmed destroyed; the parent's attack animation, cooldown, and pre-spawn effects can still run. Existing vanilla spawn restrictions still apply. Parent/child tracking survives world saves and network ownership changes; an unloaded or not-yet-synchronized child is not treated as dead. A remote destruction confirmation may make capacity available on the next summon attempt.

This support starts with newly cast direct summons; resummon existing casters to enable it. Wild or unrelated tamed casters are unaffected, and support does not propagate to grandchildren or non-`SpawnAbility` spawning systems. Child levels, drops, lifetime and removal when their parent dies continue to follow the original creature behavior; the staff's quality/lifetime settings are not propagated to them.
